...
try {
    ...
    foreach ($label in @('host','client-1','client-2','client-3')) {
        ...
        if ($outputText -notmatch 'peak=4' -or $outputText -notmatch 'pipe=0.5' -or $outputText -notmatch 'voice=[1-9]') { throw "$label failed: $outputText" }
        Write-Output "$label PASS: four peers, replicated job state, voice packets"
    }
} finally {
